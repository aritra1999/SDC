class Car {
    ...
    update(roadBorders, traffic) {
        if(!this.damaged) {
            ...
            this.damaged = this.#asseccDamage(roadBorders, traffic);        
        }
        if(this.sensor) {
    ...
    #asseccDamage(roadBorders, traffic) {
        for(let i=0; i<roadBorders.length; i++) {
            if(polgonIntersect(this.polygon, roadBorders[i]))return true;
        }

        for(let i=0; i<traffic.length; i++) {
            if(polgonIntersect(this.polygon, traffic[i].polygon))return true;
        }
